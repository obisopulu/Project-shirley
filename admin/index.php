...
if($e_act == "Login" && $_SERVER["REQUEST_METHOD"] == "POST" ){
	
	if($e_name == "durambon" &&	$e_key == "durambop"){
		
		$_SESSION['sessionAdminer'] = "on";
		header("Location: index.php");
	
	}
	
}
if($e_act == "Logout" && $_SERVER["REQUEST_METHOD"] == "GET" ){
	
	$_SESSION['sessionAdminer'] = "";
	header("Location: index.php");
}
...
<head>
<meta charset="utf-8">
<title><?php if ($sessionAdminer != "on"){echo "Admin";}else{echo "Today";}?></title>
...
<body>
<?php if ($sessionAdminer != "on"){?>
	<div style="margin:10px; padding:0 10px; margin-bottom:50px;" align="center">
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"])?>" method="post">
    <a href="../index.php"><h1 style="font-size:4em; margin:0">breeders</h1></a>
    <div style="width:250px; padding:10px 0; margin:10px 0">
    <br><br><br><br><br><br>Admin<br><br><br>
    <div style="font-size:.8em">
    <?php if ($login_error) {echo "<br><br>$login_error";}?>
    </div>
    <input type="password" name="e_name" placeholder="eName" style="background:rgba(0,0,0,.2); border:none; height:20px; padding:10px; margin:10px; width:85%"/>
    <input type="password" name="e_key" placeholder="eKey" style="background:rgba(0,0,0,.2); border:none; height:20px; padding:10px; margin:10px; width:85%"/>
    <input type="submit" name="e_act" value="Login" style="width:200px; height:40px; background:#0C9BBB; font-size:1em; border:none"></form>
    </div>
    </div>
<?php die();}else{?>
...
$sqlUserCount1 = mysqli_query($cxn, "SELECT DISTINCT anonymousIP FROM anonymouser WHERE anonymousTime <= 200 AND dateUpdated = '$today' ");
$sqlUserCount2 = mysqli_query($cxn, "SELECT DISTINCT anonymousIP FROM anonymouser WHERE anonymousTime > 200 AND anonymousTime <= 400 AND dateUpdated = '$today' ");
$sqlUserCount3 = mysqli_query($cxn, "SELECT DISTINCT anonymousIP FROM anonymouser WHERE anonymousTime > 400 AND anonymousTime <= 600 AND dateUpdated = '$today' ");
$sqlUserCount4 = mysqli_query($cxn, "SELECT DISTINCT anonymousIP FROM anonymouser WHERE anonymousTime > 600 AND anonymousTime <= 800 AND dateUpdated = '$today' ");
$sqlUserCount5 = mysqli_query($cxn, "SELECT DISTINCT anonymousIP FROM anonymouser WHERE anonymousTime > 800 AND anonymousTime <= 1000 AND dateUpdated = '$today' ");
$sqlUserCount6 = mysqli_query($cxn, "SELECT DISTINCT anonymousIP FROM anonymouser WHERE anonymousTime > 1000 AND anonymousTime <= 1200 AND dateUpdated = '$today' ");
$sqlUserCount7 = mysqli_query($cxn, "SELECT DISTINCT anonymousIP FROM anonymouser WHERE anonymousTime > 1200 AND anonymousTime <= 1400 AND dateUpdated = '$today' ");
$sqlUserCount8 = mysqli_query($cxn, "SELECT DISTINCT anonymousIP FROM anonymouser WHERE anonymousTime > 1400 AND anonymousTime <= 1600 AND dateUpdated = '$today' ");
$sqlUserCount9 = mysqli_query($cxn, "SELECT DISTINCT anonymousIP FROM anonymouser WHERE anonymousTime > 1600 AND anonymousTime <= 1800 AND dateUpdated = '$today' ");
$sqlUserCount10 = mysqli_query($cxn, "SELECT DISTINCT anonymousIP FROM anonymouser WHERE anonymousTime > 1800 AND anonymousTime <= 2000 AND dateUpdated = '$today' ");
$sqlUserCount11 = mysqli_query($cxn, "SELECT DISTINCT anonymousIP FROM anonymouser WHERE anonymousTime > 2000 AND anonymousTime <= 2200 AND dateUpdated = '$today' ");
$sqlUserCount12 = mysqli_query($cxn, "SELECT DISTINCT anonymousIP FROM anonymouser WHERE anonymousTime > 2200 AND dateUpdated = '$today' ");
...
<?php
$sql_users = mysqli_num_rows(mysqli_query($cxn, "SELECT DISTINCT anonymousIP FROM anonymouser WHERE dateUpdated = '$today' "))*1;
$sql_pages = mysqli_num_rows(mysqli_query($cxn, "SELECT * FROM anonymouser WHERE dateUpdated = '$today' "))*1;
$sql_device = mysqli_num_rows(mysqli_query($cxn, "SELECT DISTINCT anonymousDevice FROM anonymouser WHERE dateUpdated = '$today' "))*1;
$sql_from = mysqli_num_rows(mysqli_query($cxn, "SELECT DISTINCT anonymousFrom FROM anonymouser WHERE dateUpdated = '$today' "))*1;
$sql_mobile = mysqli_num_rows(mysqli_query($cxn, "SELECT * FROM anonymouser WHERE anonymousPlatform = 'mobile' AND dateUpdated = '$today' "))*1;
$sql_desktop = mysqli_num_rows(mysqli_query($cxn, "SELECT * FROM anonymouser WHERE anonymousPlatform = 'desktop' AND dateUpdated = '$today' "))*1;
$sql_morning = mysqli_num_rows(mysqli_query($cxn, "SELECT DISTINCT anonymousIP FROM anonymouser WHERE anonymousTime < '1200' AND dateUpdated = '$today' "))*1;
$sql_afternoon = mysqli_num_rows(mysqli_query($cxn, "SELECT DISTINCT anonymousIP FROM anonymouser WHERE anonymousTime > '1200' AND anonymousTime < '1900' AND dateUpdated = '$today' "))*1;
$sql_night = mysqli_num_rows(mysqli_query($cxn, "SELECT DISTINCT anonymousIP FROM anonymouser WHERE anonymousTime > '1900' AND dateUpdated = '$today' "))*1;

$sql_seller = mysqli_num_rows(mysqli_query($cxn, "SELECT * FROM seller WHERE dateUpdated = '$today' "))*1;
$sql_posts_active = mysqli_num_rows(mysqli_query($cxn, "SELECT * FROM poster WHERE postStatus = 'Live' AND dateUpdated = '$today' "))*1;
$sql_posts_review = mysqli_num_rows(mysqli_query($cxn, "SELECT * FROM poster WHERE postStatus = 'Under Review' AND dateUpdated = '$today' "))*1;
$sql_posts_inactive_sold = mysqli_num_rows(mysqli_query($cxn, "SELECT * FROM poster WHERE postStatus = 'Live' AND dateUpdated = '$today' "))*1;

$sql_device_android = mysqli_num_rows(mysqli_query($cxn, "SELECT DISTINCT anonymousIP FROM anonymouser WHERE anonymousDevice = 'Android' AND dateUpdated = '$today' "))*1;
$sql_device_ios = mysqli_num_rows(mysqli_query($cxn, "SELECT DISTINCT anonymousIP FROM anonymouser WHERE anonymousDevice = 'iOS' AND dateUpdated = '$today' "))*1;
$sql_device_windows = mysqli_num_rows(mysqli_query($cxn, "SELECT DISTINCT anonymousIP FROM anonymouser WHERE anonymousDevice = 'Windows' AND dateUpdated = '$today' "))*1;
$sql_device_meego = mysqli_num_rows(mysqli_query($cxn, "SELECT DISTINCT anonymousIP FROM anonymouser WHERE anonymousDevice = 'MeeGo' AND dateUpdated = '$today' "))*1;
$sql_device_linux = mysqli_num_rows(mysqli_query($cxn, "SELECT DISTINCT anonymousIP FROM anonymouser WHERE anonymousDevice = 'Linux' AND dateUpdated = '$today' "))*1;
