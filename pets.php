...
mysqli_query($cxn, "INSERT INTO anonymouser(anonymousID, anonymousIP, anonymousPage, anonymousPlatform, anonymousDevice, anonymousTime, dateUpdated) VALUES (NULL, '$userIP', 'Pet', '$anonymousPlatform', '$anonymousDevice', '$current_time_day','$today')") or die("Home");



$seo_title = "Buy or Adopt Pets in Nigeria - breedersng";
$seo_description = "Buy or Adopt Pets from Nigeria's online breeders Marketplace, or sell";
$seo_keywords = "buy, adopt, sell, pet, livestock, feed, accessory, nigeria, farm, dog, cat,  goat, sheep, poultry, puppies, horse, cow, chiken, mice, money";
$seo_img = "img/banner.png";
$seo_url = "https://www.breeders.ng/pets/";
$seo_author = "breeders";
$seo_date = "$today";
...
<div id="page_pic" class="page_pic_p"></div>
...
<?php $cat = "Pet"; include("services/footer_head.php");?>
...
<div id="match_screen_width" class="pets" align="center">
<table id="scroller_table"><tr>

<?php 
$sql_frame = mysqli_query($cxn, "SELECT DISTINCT postSubCategory FROM poster WHERE postCategory = 'Pet' AND postSubCategory != '' ORDER BY RAND() LIMIT 10");
...
<div id="seo_description" class="pets">
...
$sql_poster = mysqli_query($cxn, "SELECT * FROM poster WHERE postStatus = 'Live' AND postCategory = 'Pet' ");
...
$sql_poster = mysqli_query($cxn, "SELECT * FROM poster WHERE postStatus = 'Live' AND postCategory = 'Pet' ORDER BY postID DESC LIMIT $pageno, 20");
...
<div align="center">
<ul>
	<?php $x = $current_page - 1; if ($current_page > 1){ ?><li id="pagination_pets" onClick="location = '<?php echo $_SERVER["PHP_SELF"]."?p='+$x"?>" 
	> |< </li><?php }else {echo "<li id='pagination_pets' > .... </li>";}?>
	<select id="pagination_pets" name="p" onchange="location = '<?php echo $_SERVER["PHP_SELF"]; ?>?p='+this.value;">
	<?php 
	...
	<?php if ($current_page < $count_page ){?><li id="pagination_pets" <?php $current_page++;?>
