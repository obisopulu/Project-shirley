...
$seo_img = "https://www.breeders.ng/postimg/$postPic1";
$seo_url = "https://www.breeders.ng/preview.php?cat=$postCategory&id=$postID";
$seo_author = "breeders";
$seo_date = "$today";
...
<?php
	$counter = 1;
$sql_poster1 = mysqli_query($cxn, "SELECT * FROM poster WHERE postStatus = 'Live' AND postCategory = '$postCategory' AND postID != '$jsPostID' ORDER BY RAND() LIMIT 5");

if(mysqli_num_rows($sql_poster) * 1 < 2) {$sql_poster1 = mysqli_query($cxn, "SELECT * FROM poster WHERE postStatus = 'Live' AND postID != '$jsPostID' ORDER BY RAND() LIMIT 5");}

while($row_poster1=mysqli_fetch_assoc($sql_poster1)){
	extract($row_poster1);
$linkTitle = $postTitle;
$linkTitle = clean_input2($linkTitle);
$link = strtolower("/preview.php?cat=$postCategory&id=$postID");
?>
