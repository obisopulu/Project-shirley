...
mysqli_query($cxn, "INSERT INTO anonymouser(anonymousID, anonymousIP, anonymousPage, anonymousPlatform, anonymousDevice, anonymousTime, dateUpdated) VALUES (NULL, '$userIP', 'FeedandStuff', '$anonymousPlatform', '$anonymousDevice', '$current_time_day','$today')") or die("Home");


$seo_title = "Buy Animal Food/Feed and Accessories - @breedersng";
$seo_description = "Buy Animal Food/Feed and Accessories from Nigeria's online breeders Marketplace, or sell";
$seo_keywords = "cage, chain, feeder, buy, sell, pet, livestock, feed, accessory, nigeria, farm, dog, cat,  goat, sheep, poultry, puppies, horse, cow, chiken, mice, money";
$seo_img = "img/banner.png";
$seo_url = "https://www.breeders.ng/feedandstuff/";
$seo_author = "breeders";
$seo_date = "$today";
...
<div id="page_pic" class="page_pic_f"></div>
...
<?php $cat = "Feed"; include("services/footer_head.php");?>
...
<div id="match_screen_width" class="feedandstuff" align="center">
...
<div id="seo_description" class="feedandstuff">
...
$sql_poster = mysqli_query($cxn, "SELECT * FROM poster WHERE postStatus = 'Live' AND (postCategory = 'Feed' OR postCategory = 'Accessory') ");
...
$sql_poster = mysqli_query($cxn, "SELECT * FROM poster WHERE postStatus = 'Live' AND (postCategory = 'Feed' OR postCategory = 'Accessory') ORDER BY postID DESC LIMIT $pageno, 20");
...
<div align="center">
<ul>
	<?php $x = $current_page - 1; if ($current_page > 1){ ?><li id="pagination_feedandstuff" onClick="location = '<?php echo $_SERVER["PHP_SELF"]."?p='+$x"?>" 
	> |< </li><?php }else {echo "<li id='pagination_feedandstuff' > .... </li>";}?>
	<select id="pagination_feedandstuff" name="p" onchange="location = '<?php echo $_SERVER["PHP_SELF"]; ?>?p='+this.value;">
	<?php 
	...
	<?php if ($current_page < $count_page ){?><li id="pagination_feedandstuff" <?php $current_page++;?>
